...
void RemoteControl::sendMainMenu(boolean update){
  ...
  serialPort->print(F("|r~Commands|n~Manual|s~Settings|A~Live Data |β~Custom |in~Info|c~Test compass|yp~Plot"));
...
void RemoteControl::sendCustomMenu(boolean update){
  if (update) serialPort->print("{:"); else serialPort->print(F("{.Custom"));
  serialPort->print(F("{.Ardumower"));
  serialPort->print(" (");
  serialPort->print(robot->name);
  serialPort->print(")");  
...
void RemoteControl::sendSettingsMenu(boolean update){
  ...
  serialPort->print(F("|sz~Save settings|s1~Motor|s2~Mow|s16~Free wheel|s3~BumperDuino|s4~Sonar|s5~Perimeter|S5~Perimeter2|s6~Lawn sensor|s7~IMU|S7~IMU2|s8~R/C"));
...
//FR: custom
void RemoteControl::sendPerimeterMenu2(boolean update){
  if (update) serialPort->print("{:"); else serialPort->print(F("{.Perimeter2`1000"));  
  serialPort->println(F("|E02~Value-L "));
  serialPort->print(robot->perimeterMagleft);
  if (robot->perimeterMagleft < 0) serialPort->print(" (inside)");
    else serialPort->print(" (outside)");
  serialPort->println(F("|E03~Value-R "));
  serialPort->print(robot->perimeterMagright);
  if (robot->perimeterMagright < 0) serialPort->print(" (inside)");
    else serialPort->print(" (outside)");  
    
  serialPort->print(F("|E22~Smag-L "));
  serialPort->print(robot->perimeter.getSmoothMagnitude(0));
  serialPort->print(F("|E24~Smag-R "));
  serialPort->print(robot->perimeter.getSmoothMagnitude(1));
  serialPort->print(F("|E23~Timeout-L "));
  serialPort->print(robot->perimeter.signalTimedOut(0));
  
  serialPort->print(F("|E27~Use Softborder left "));
  sendYesNo(robot->use_leftSoftBorder);
  Console.print("use_leftSoftBorder=");
      Console.println(robot->use_leftSoftBorder);
  serialPort->print(F("|E28~Use Softborder right  "));
  sendYesNo(robot->use_rightSoftBorder);
  Console.print("use_rightSoftBorder=");
      Console.println(robot->use_rightSoftBorder);
  serialPort->print(F("|E29~Use Drahtfolgen  "));
  sendYesNo(robot->use_drahtfolgenIUZ);
...
//FR: custom
void RemoteControl::processPerimeterMenu2(String pfodCmd){ 
  if (pfodCmd.startsWith("E25")) robot->saveUserSettings();  
    else if (pfodCmd.startsWith("E27")) robot->use_leftSoftBorder = !robot->use_leftSoftBorder;
    else if (pfodCmd.startsWith("E28")) robot->use_rightSoftBorder = !robot->use_rightSoftBorder;
    else if (pfodCmd.startsWith("E29")) robot->use_drahtfolgenIUZ = !robot->use_drahtfolgenIUZ;
...
void RemoteControl::sendPerimeterMenu(boolean update){
  ...
  sendSlider("e26", F("MaxSpeedperiPwm"), robot->MaxSpeedperiPwm, "", 1, 255);        
  sendPIDSlider("e07", F("Track"), robot->perimeterPID, 0.1, 100);  
  //serialPort->print(F("|e09~Use differential signal "));sendYesNo(robot->perimeter.useDifferentialPerimeterSignal);    
...
void RemoteControl::processPerimeterMenu(String pfodCmd){        
    ...
    else if (pfodCmd.startsWith("e26")) processSlider(pfodCmd, robot->MaxSpeedperiPwm, 1);  
    else if (pfodCmd.startsWith("e19")) robot->setNextState(STATE_OFF, 0);          
		else if (pfodCmd.startsWith("e20")) robot->setNextState(STATE_PERI_FIND, 0);                      
		else if (pfodCmd.startsWith("e21")) robot->setNextState(STATE_PERI_TRACK, 0);                          
...
//FR:custom //Wendewinkel
void RemoteControl::sendImuMenu2(boolean update){
  ...
  serialPort->print(F(" deg"));  
  sendSlider("G10", F("imuDriveHeadingMin"), robot->imuDriveHeadingMin, "", 1, 180);
  sendSlider("G11", F("imuDriveHeadingMax"), robot->imuDriveHeadingMax, "", 1, 180);        
  serialPort->println("}");
}
// FR: imuDriveHeadingMin, imuDriveHeadingMax sind kein Standard daher
  // definieren in robot.h
  // ------- IMU state  --------------------------------
  //  float imuDriveHeading ;       // standard: drive heading (IMU)
  //  float imuRollHeading ;        // standard: roll heading  (IMU)
  //  unsigned long imuDriveHeadingMin;     //FR: min-Winkel beim Roll (Random-Untergrenze)
  //  unsigned long imuDriveHeadingMax;     //FR: max-Winkel beim Roll (Random-Obergrenze)

  // unter "Robot::Robot(){" auf Standardwerte setzen
  // imuDriveHeading = 0;
  // imuRollHeading  = 0;        
  // imuDriveHeadingMin = 90;     
  // imuDriveHeadingMax =180;   

  // in settings.h unter "Robot::loadSaveUserSettings" eintragen
  // eereadwrite(readflag, addr, imuRollPID.Ki);    // standard:
  // eereadwrite(readflag, addr, imuRollPID.Kd);    // standard:
  // eereadwrite(readflag, addr, imuDriveHeadingMin); //FR:
  // eereadwrite(readflag, addr, imuDriveHeadingMax); //FR:   

//FR:custom
void RemoteControl::processImuMenu2(String pfodCmd){      
  if (pfodCmd == "G00" ) robot->imuUse = !robot->imuUse;
    else if (pfodCmd.startsWith("G10")) processSlider(pfodCmd, robot->imuDriveHeadingMin, 1);  
    else if (pfodCmd.startsWith("G11")) processSlider(pfodCmd, robot->imuDriveHeadingMax, 1);   
...
void RemoteControl::sendCompassMenu(boolean update){
  ...
  serialPort->print(robot->imu.ypr.yaw/PI*180);
  serialPort->print(F("|cs~South|cm~Mow"));
  serialPort->print(F("|cl~5 Grad "));
  serialPort->print(F("|cr~5 Grad "));
  serialPort->print(F("|cf~Forward "));
  sendSlider("ct", F("Set Heading"), robot->imuRollHeading/PI*180, "", 1, 360);        
  serialPort->print(F("|cy~"));
  serialPort->print(robot->imu.ypr.yaw/PI*180);
  serialPort->print(F("|ch~"));
  printDouble(robot->imuRollHeading/PI*180,2);  
  serialPort->println("}");
}
...
void RemoteControl::processCompassMenu(String pfodCmd){
 if (pfodCmd == "cm"){
    ...
    robot->imuDriveHeading -= PI/36;    
    robot->imuRollHeading -= PI/36;             
    sendCompassMenu(true);
  } else if (pfodCmd == "cr"){
    robot->imuDriveHeading += PI/36;   
    robot->imuRollHeading += PI/36;            
    ...
    robot->setNextState(STATE_FORWARD, 0);            
    sendCompassMenu(true);    
  } else if (pfodCmd.startsWith("ct")){
    float dummy = robot->imuRollHeading/PI*180;    
    processSlider(pfodCmd, dummy, 1);
    robot->imuRollHeading = dummy / 180*PI;
    robot->imuDriveHeading = robot->imuRollHeading;  
    Console.print("dummy=");Console.print(dummy);
    Console.print("robot->imuRollHeading=");Console.print(robot->imuRollHeading);
    sendCompassMenu(true); 
  }
  sendCompassMenu(true);
}
...
void RemoteControl::processSettingsMenu(String pfodCmd){   
  if (pfodCmd == "s1") sendMotorMenu(false);
      ...
      else if (pfodCmd == "S5") sendPerimeterMenu2(false);//großes S
      else if (pfodCmd == "s6") sendLawnSensorMenu(false);
      else if (pfodCmd == "s7") sendImuMenu(false);
      else if (pfodCmd == "S7") sendImuMenu2(false);//großes S !
...
void RemoteControl::processLiveDataBatteryMenu(boolean update){  
  ...
  serialPort->print(F("|pl1~Battery "));
  serialPort->print(robot->batVoltage);  
  serialPort->print(F(" V"));
  serialPort->println("}");
} 

//LiveData Perimeter Custom
void RemoteControl::processLiveDataPerimeterMenu(boolean update){  
  ...
  serialPort->print(robot->perimeterMagright);
  serialPort->print(F("|pl3~Smag_L  "));
  serialPort->print(robot->perimeter.getSmoothMagnitude(0));
  serialPort->print(F("|pl4~Smag_R  "));
  serialPort->print(robot->perimeter.getSmoothMagnitude(1));  
  serialPort->print(F("|pl5~Timeout  "));
  serialPort->print(robot->perimeter.signalTimedOut(0));  
  serialPort->print(F("|pl6~PID_X  "));
  serialPort->print(robot->PIDX);  
  serialPort->print(F("|pl7~PID_Y  "));
  serialPort->print(robot->perimeterPID.y);    
  serialPort->println("}");
}
...
void RemoteControl::processLiveDataPerimeterMenu(boolean update){  
  ...
  serialPort->print(robot->perimeter.getSmoothMagnitude(0));
  serialPort->print(F("|pl3~Timeout  "));
  serialPort->print(robot->perimeter.signalTimedOut(0));  
  serialPort->print(F("|pl4~PID_Y  "));
  serialPort->print(robot->perimeterPID.y);    
  serialPort->println("}");
}
*/
...
void RemoteControl::run(){  
  serialPort->print("RemoteControl Run");
}
...
bool RemoteControl::readSerial(){
  ...
  while(serialPort->available() > 0){
    ...
    if (pfodCmdComplete) {
      ...
      else if (pfodCmd == "B") sendCustomMenu(false);  
      else if (pfodCmd.startsWith("y")) {
        if (pfodCmd == "yp") sendPlotMenu(false);
      ...
      else if (pfodCmd.startsWith("S")) processSettingsMenu(pfodCmd);//großes S
      ...
      else if (pfodCmd.startsWith("E")) processPerimeterMenu2(pfodCmd);     
      else if (pfodCmd.startsWith("f")) processLawnSensorMenu(pfodCmd);       
      else if (pfodCmd.startsWith("g")) processImuMenu(pfodCmd);   
      else if (pfodCmd.startsWith("G")) processImuMenu2(pfodCmd);       
